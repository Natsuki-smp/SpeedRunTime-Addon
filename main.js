...
function rtaIgtText(rtaMs, igtMs) {
  return `§c§lRTA (SERVER): §f${formatMs(rtaMs)}\n§e§lIGT: §f${formatMs(igtMs)}`;
}
...
function finishSharedRta(finisher) {
  ...
  world.sendMessage(`§d✦ §l§d${finisher.name} §r§f➤ §aWIN! §c RTA SERVER: §f${formatMs(finalMs)}`);
  for (const p of world.getPlayers()) {
    ...
    p.onScreenDisplay.updateSubtitle(`§cRTA SERVER: §f${formatMs(finalMs)}`);
    p.playSound("random.levelup");
  }
}
